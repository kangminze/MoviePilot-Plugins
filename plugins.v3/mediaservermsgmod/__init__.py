...
from app.schemas.types import EventType, MediaType, MediaImageType, MediaSource, NotificationType
...
class MediaServerMsgMod(_PluginBase):
    ...
    def send(self, event: Event):
        ...
        try:
            ...
            media_type = MediaType.MOVIE if item_type == "MOV" else MediaType.TV
    ...
    def _send_aggregated_message(self, series_id: str):
        ...
        try:
            ...
            # 路径中明确标注 TMDB 时，写入成对的统一媒体身份。
            tmdb_pattern = r'[\[{](?:tmdbid|tmdb)[=-](\d+)[\]}]'
            match = re.search(tmdb_pattern, first_event.item_path or "")
            if match:
                first_event.media_source = MediaSource.TMDB
                first_event.media_id = match.group(1)
                logger.info(f"从路径提取到 TMDB 身份: {first_event.media_id}")
            elif not first_event.media_source or not first_event.media_id:
                logger.info(f"未从路径中提取到 TMDB 身份: {first_event.item_path}")
                logger.info("尝试从媒体服务获取统一媒体身份")
                ...
                if media_service:
                    ...
                    if service and item_id:
                        ...
                        if info and info.media_source and info.media_id:
                            first_event.media_source = info.media_source
                            first_event.media_id = str(info.media_id)
                            logger.info(
                                f"从媒体服务器获取到媒体身份: "
                                f"{info.media_source}:{info.media_id}"
                            )
                        else:
                            logger.info("媒体服务器未返回完整媒体身份")
            ...
            try:
                if not tmdb_id:
                    logger.debug("无法获得 TMDB 身份，安全保留原有消息")
                    # 使用原有逻辑构造消息
    ...
    def _merge_continuous_episodes(self, events: List[WebhookEventInfo]) -> str:
        ...
        # 只有直接或显式转换得到的 TMDB 身份才能查询 TMDB 详情。
        tmdb_info = {}
        try:
            if events:
                tmdb_id = self._resolve_tmdb_id(
    ...
    def _resolve_tmdb_id(
        ...
        try:
            ...
            converted_id = converted.get("id") if converted else None
            if converted_id:
                logger.info(
                    f"媒体身份已转换为 TMDB: {media_source}:{media_id} -> {converted_id}"
                )
                return str(converted_id)
        except Exception as e:
            logger.warning(
                f"媒体身份转换 TMDB 失败，保留原消息: {media_source}:{media_id}, {e}"
            )
            return None

        logger.info(f"媒体身份无法转换为 TMDB，保留原消息: {media_source}:{media_id}")
        return None
    ...
    def _get_tmdb_info(self, tmdb_id: str, mtype: MediaType, season: Optional[int] = None):
        ...
        if mtype == MediaType.MOVIE:
            return self.chain.tmdb_info(tmdbid=normalized_id, mtype=mtype)

        tmdb_info = self.chain.tmdb_info(
            tmdbid=normalized_id, mtype=mtype, season=season
        ) or {}
        tmdb_info2 = self.chain.tmdb_info(tmdbid=normalized_id, mtype=mtype) or {}
