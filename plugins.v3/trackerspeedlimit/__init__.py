...
class TrackerSpeedLimit(_PluginBase):
    ...
    def _speed_limit(self, _hash=None):
        """
        补全下载历史的标签与分类
        """
        ...
        for service in self.service_infos.values():
            ...
            for torrent in torrents:
                try:
                    ...
                    torrent_key = self._torrent_key(torrent=torrent, dl_type=service.type)
                    if not torrent_key:
                        logger.warning(f"{self.LOG_TAG}种子名称或大小缺失，跳过限速处理")
                        continue
                    _size, _name = torrent_key
                    # 获取种子hash
    ...
    @staticmethod
    def _get_label(torrent: Any, dl_type: str) -> List[str]:
        """获取种子现有标签。"""
        try:
            if dl_type == "qbittorrent":
                raw_tags = torrent.get("tags", "")
            else:
                raw_tags = torrent.labels or []
            if isinstance(raw_tags, str):
                raw_tags = raw_tags.split(",")
            return [str(tag).strip() for tag in raw_tags if str(tag).strip()]
        except Exception as err:
            logger.warning(f"获取种子标签失败：{err}")
            return []
    ...
    def download_added(self, event: Event):
        ...
        try:
            ...
            if not context or not context.torrent_info or not _hash:
                logger.info("触发添加下载事件，但上下文或种子哈希不完整，跳过后续处理")
                return
