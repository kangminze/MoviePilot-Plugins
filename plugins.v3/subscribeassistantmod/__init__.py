...
from app.sdk.media import Context, MediaInfo, MetaInfo, TorrentInfo, build_media_key, resolve_media_identity
...
from app.schemas.types import EventType, ChainEventType, MediaType, MediaSource, NotificationType
...
class SubscribeAssistantMod(_PluginBase):
    ...
    def handle_subscribe_added_event(self, event: Event = None):
        ...
        try:
            ...
            if not subscribe or not self.__media_task_key(subscribe):
                logger.warning(f"订阅 ID {subscribe_id} 不存在或媒体身份不完整，跳过处理")
                return
    ...
    def handle_subscribe_complete_event(self, event: Event = None):
        ...
        try:
            ...
            mediainfo = MediaInfo()
            mediainfo.from_dict(mediainfo_dict)
            if not self.__media_task_key(mediainfo):
                logger.warning("订阅完成事件媒体身份不完整，跳过洗版处理")
                return

            # 分类是 TMDB 专属增强；转换失败不阻断后续洗版主流程。
            if self._tv_episode_exclude_type:
                logger.debug(f"剧集分集类型排除已设置，跳过类别:{self._tv_episode_exclude_type}")
                mtype = MediaType.TV if mediainfo.type == MediaType.TV else MediaType.MOVIE
                tmdb_id = self.__tmdb_media_id(mediainfo, mtype=mtype, season=mediainfo.season)
                info = self.tmdb.get_info(mtype=mtype, tmdbid=tmdb_id) if tmdb_id else None
                cat = None
                if info:
                    cat = self.category.get_tv_category(info) \
                        if info.get('media_type') == MediaType.TV else self.category.get_movie_category(info)
                elif tmdb_id:
                    logger.warning(f'{mediainfo.title} 未获取到 TMDB 信息')

                if cat and str(cat) in self._tv_episode_exclude_type:
                    logger.debug(f"剧集分集类型{str(cat)}被排除，跳过自动洗版处理")
                    return
                if tmdb_id and not cat:
                    logger.warning(f'{mediainfo.title} 未获取到二级分类信息')
    ...
    def __handle_resource_download_history_clear(self, subscribe: Subscribe):
        ...
        media_source, media_id = self.__resolve_identity(subscribe)
        if not media_source or not media_id:
            logger.warning(f"{self.__format_subscribe(subscribe)} 媒体身份不完整，跳过整理记录和源文件清理")
            return

        season = subscribe.season if subscribe_type == MediaType.TV else None
        histories = self.transferhistory_oper.get_by(
            media_source=media_source,
            media_id=media_id,
            mtype=subscribe.type,
            season=season,
        )
        media_key = self.__media_task_key(subscribe)
        if not histories:
            logger.info(f"{self.__format_subscribe(subscribe)} {media_key} 未匹配到整理记录，跳过处理")
            return

        logger.info(f"即将开始清理洗版资源整理源文件记录")
        logger.info(f"{self.__format_subscribe(subscribe)} {media_key} 获取到 {len(histories)} 条整理记录，即将开始清理")
    ...
    def __clear_transfer_src_histories(self, tasks: dict, subscribe: Subscribe,
        ...
        """
        clear_key = self.__media_task_key(subscribe)
        if not clear_key or not histories:
            logger.warning("媒体身份不完整，跳过整理记录、源文件和清理任务写入")
            return

        tasks[clear_key] = {
            "subscribe_id": subscribe.id,
            "media_source": str(subscribe.media_source),
        ...
        for history in histories:
            ...
            # 删除源文件失败时保留整理记录，避免把仍存在的文件标记为已清理。
            if history.src_fileitem:
                ...
                if not state:
                    logger.warning(f"{src_fileitem.path} 删除失败，保留整理记录等待重试")
                    continue
    ...
    def __handle_transfer_intercept_history_clear(self, mediainfo: MediaInfo, target_path: Path):
        ...
        clear_key = self.__media_task_key(mediainfo)
        if not clear_key:
            logger.warning("整理拦截事件媒体身份不完整，跳过媒体库文件和历史清理")
            return
    ...
    def __clear_transfer_dest_histories(self, task: dict, mediainfo: MediaInfo, target_path: Path) -> bool:
        ...
        media_key = self.__media_task_key(mediainfo)
        if not media_key:
            logger.warning("媒体身份不完整，跳过媒体库文件清理")
            return False
        logger.info(f"{subscribe_desc} {media_key} 获取到 {len(histories)} 条整理记录，即将开始清理")
    ...
    def __get_subscribe_by_source(self, source: str) -> Tuple[Optional[dict], Optional[Subscribe]]:
        ...
        subscribe = self.subscribe_oper.get(subscribe_id)
        if not subscribe or not self.__media_task_key(subscribe):
            logger.warning(f"订阅来源 ID {subscribe_id} 缺少完整媒体身份，跳过种子处理")
            return None, None
        # 事件 source 仅用于定位订阅；后续任务保存数据库中的 V3 规范快照。
        return subscribe.to_dict(), subscribe
    ...
    def __process_download_task(self, subscribe_tasks: dict, torrent_tasks: dict):
        ...
        for torrent_hash, torrent_task in list(torrent_tasks.items()):
            ...
            if is_completed:
                ...
                deleted = self.__delete_torrents(downloader=service.instance, torrent_hashes=torrent_hash)
                if not deleted:
                    logger.warning(f"种子任务 {torrent_desc} 删除失败，保留任务等待重试")
                    continue
    ...
    def __process_subscribe_pause_for_download(self, subscribe_task: dict, subscribe: Subscribe,
        ...
        if action == "pause":
            ...
            media_source, media_id = self.__resolve_identity(subscribe)
            if not media_source or not media_id:
                logger.warning(f"{self.__format_subscribe(subscribe)} 媒体身份不完整，跳过破坏性订阅操作")
                return False
            if action == "complete":
    ...
    def __recognize_media(self, subscribe: Subscribe) -> Optional[MediaInfo]:
        ...
        media_source, media_id = self.__resolve_identity(subscribe)
        if not media_source or not media_id:
            logger.warning(f"订阅 ID {subscribe.id} 媒体身份不完整，跳过识别")
            return None
        try:
            # 显式身份识别，禁止退化为标题匹配。
            ...
            if not mediainfo:
                logger.warning(f"未识别到媒体信息：{build_media_key(media_source, media_id)}")
                return None
            return mediainfo
        except Exception as e:
    ...
    @staticmethod
    def __resolve_identity(media: Any) -> Tuple[Optional[MediaSource], Optional[str]]:
        """解析完整的 V3 媒体身份；任务字典只接受规范字段。"""
    ...
    @classmethod
    def __media_task_key(cls, media: Any, include_season: bool = True) -> str:
        """构造插件任务键，电视剧任务按季隔离。"""
        media_source, media_id = cls.__resolve_identity(media)
        media_key = build_media_key(media_source, media_id)
        if not media_key:
            return ""
        season = getattr(media, "season", None) if not isinstance(media, dict) else media.get("season")
        if include_season and season is not None:
            return f"{media_key}:season:{season}"
        return media_key

    def __tmdb_media_id(self, media: Any, mtype: Optional[MediaType] = None,
                        season: Optional[int] = None) -> Optional[str]:
        """为 TMDB 专属接口显式转换身份；转换失败不影响主流程。"""
        media_source, media_id = self.__resolve_identity(media)
        if not media_source or not media_id:
            return None
        if media_source == MediaSource.TMDB:
            return media_id
        try:
            target = self.media_chain.convert_media_identity(
            ...
            logger.warning(f"媒体身份 {build_media_key(media_source, media_id)} 转换到 TMDB 失败：{err}")
            return None
        target_id = target.get("id") if isinstance(target, dict) else None
        return str(target_id) if target_id is not None else None

    @staticmethod
    def __migrate_identity_record(record: dict) -> bool:
        """迁移可明确判定来源的插件旧身份字段。"""
        ...
        if media_source and media_id:
            changed = False
            for field, _ in legacy_sources:
                if field in record:
                    record.pop(field)
                    changed = True
            return changed
        candidates = [(field, source, record.get(field)) for field, source in legacy_sources if record.get(field)]
        if len(candidates) != 1:
            if candidates:
                logger.warning("插件旧身份包含多个来源，无法安全判定，原样保留")
            return False
        _, source, legacy_id = candidates[0]
        source, media_id = resolve_media_identity(media_source=source, media_id=legacy_id)
        if not source or not media_id:
            logger.warning("插件旧身份无效，原样保留")
            return False
        record["media_source"] = source.value
        record["media_id"] = media_id
        for field, _ in legacy_sources:
            record.pop(field, None)
        return True

    def __migrate_identity_data(self) -> None:
        """幂等迁移插件自己的旧任务身份，不读取或改写宿主业务表。"""
        clear_tasks = self.__get_data(key="best_version_clear_histories")
        for old_key in list(clear_tasks):
            if not isinstance(old_key, (str, int)) or not str(old_key).isdigit():
                if ":" not in str(old_key):
                    logger.warning(f"无法判定旧洗版清理任务身份，保留 key={old_key}")
                continue
            task = clear_tasks.get(old_key)
            new_key = build_media_key(MediaSource.TMDB, old_key)
            if isinstance(task, dict) and task.get("season") is not None:
                new_key = f"{new_key}:season:{task.get('season')}"
            if new_key in clear_tasks and clear_tasks[new_key] != task:
                logger.warning(f"洗版清理任务迁移目标冲突，保留旧 key={old_key}")
                continue
            clear_tasks[new_key] = task
            self.__save_data(key="best_version_clear_histories", value=clear_tasks)
            clear_tasks.pop(old_key, None)
            self.__save_data(key="best_version_clear_histories", value=clear_tasks)

        for data_key in ("subscribes", "torrents", "deletes"):
            records = self.__get_data(key=data_key)
            changed = False
            for record in records.values():
                changed = self.__migrate_identity_record(record) or changed
                subscribe_info = record.get("subscribe_info") if isinstance(record, dict) else None
                changed = self.__migrate_identity_record(subscribe_info) or changed
            if changed:
                self.__save_data(key=data_key, value=records)
    ...
    def __match_subscribe(subscribe: Subscribe, subscribe_task: dict) -> bool:
        ...
        # 订阅记录 ID 与规范媒体身份都必须一致。
        if subscribe.id != subscribe_task.get("id"):
            return False
        subscribe_source, subscribe_media_id = resolve_media_identity(media=subscribe)
        task_source, task_media_id = resolve_media_identity(
            media_source=subscribe_task.get("media_source"),
            media_id=subscribe_task.get("media_id"),
        )
        if not subscribe_source or not subscribe_media_id or not task_source or not task_media_id:
            return False
        if subscribe_source != task_source or subscribe_media_id != task_media_id:
            return False
        return subscribe.season == subscribe_task.get("season")
    ...
    def clear_tasks(self, subscribe_id: int, subscribe: dict):
        ...
        :param subscribe: 订阅信息
        """
        if not subscribe_id or not isinstance(subscribe, dict):
            logger.warning(f"订阅 ID {subscribe_id} 缺少完整事件快照，跳过任务清理")
            return
        subscribe = dict(subscribe)
        self.__migrate_identity_record(subscribe)
        media_source, media_id = self.__resolve_identity(subscribe)
        if not media_source or not media_id:
            logger.warning(f"订阅 ID {subscribe_id} 媒体身份不完整，跳过任务清理")
            return
        self.__with_lock_and_update_subscribe_tasks(
            method=self.__clear_subscribe_tasks,
            subscribe_id=subscribe_id,
            media_source=media_source,
            media_id=media_id,
        )
        self.__with_lock_and_update_torrent_tasks(
            method=self.__clear_torrent_tasks,
            subscribe_id=subscribe_id,
            media_source=media_source,
            media_id=media_id,
        )
    ...
    def __clear_subscribe_tasks(subscribe_tasks: dict, subscribe_id: int,
                                media_source: MediaSource, media_id: str):
        """仅在订阅 ID 和规范媒体身份均匹配时清理订阅任务。"""
        subscribe_id = str(subscribe_id)
        subscribe_task = subscribe_tasks.get(subscribe_id)
        task_source, task_media_id = resolve_media_identity(
            media_source=subscribe_task.get("media_source") if isinstance(subscribe_task, dict) else None,
            media_id=subscribe_task.get("media_id") if isinstance(subscribe_task, dict) else None,
        )
        if task_source == media_source and task_media_id == media_id:
            subscribe_tasks.pop(subscribe_id, None)

    @staticmethod
    def __clear_torrent_tasks(torrent_tasks: dict, subscribe_id: int,
                              media_source: MediaSource, media_id: str):
        """仅在订阅 ID 和规范媒体身份均匹配时清理种子任务。"""
        for key in list(torrent_tasks):
            torrent_task = torrent_tasks[key]
            if str(torrent_task.get("subscribe_id")) != str(subscribe_id):
                continue
            subscribe_info = torrent_task.get("subscribe_info")
            task_source, task_media_id = resolve_media_identity(
                media_source=subscribe_info.get("media_source") if isinstance(subscribe_info, dict) else None,
                media_id=subscribe_info.get("media_id") if isinstance(subscribe_info, dict) else None,
            )
            if task_source == media_source and task_media_id == media_id:
    ...
    def __initialize_subscribe_task(self, subscribe: Subscribe, subscribe_tasks: dict) -> tuple[dict, bool]:
        ...
        media_source, media_id = self.__resolve_identity(subscribe)
        if not media_source or not media_id:
            logger.warning(f"订阅 ID {getattr(subscribe, 'id', None)} 身份不完整，不创建种子任务")
            return {}, False
    ...
    def process_best_version(self, subscribe_dict: dict, mediainfo: MediaInfo):
        ...
        subscribe_dict = dict(subscribe_dict)
        subscribe_id = subscribe_dict.pop("id", None)
        model_fields = set(getattr(SchemaSubscribe, "model_fields", {}))
        ...
        fields_to_pop = [
            "name", "year", "type", "media_source", "media_id",
        ...
        # SDK 暂无订阅写入出口，复用宿主应用层的完整媒体到订阅字段翻译。
        sid, err_msg = add_subscribe(
            mediainfo=mediainfo,
            subscribe_oper=self.subscribe_oper,
            **subscribe_dict,
        )
    ...
    def __check_tv_season_completed(self, mediainfo: MediaInfo, season: int) -> bool:
        ...
        tmdb_id = self.__tmdb_media_id(mediainfo, mtype=MediaType.TV, season=season)
        if not tmdb_id:
            logger.warning(f"{mediainfo.title} 无法转换为 TMDB 身份，跳过完结状态增强")
            return False
        episodes = self.tmdb_chain.tmdb_episodes(tmdbid=tmdb_id, season=season)
        if not episodes:
            return False
    ...
    def __get_tv_latest_episode(self, mediainfo: MediaInfo, season: int) -> \
        ...
        tmdb_id = self.__tmdb_media_id(mediainfo, mtype=MediaType.TV, season=season)
        if not tmdb_id:
            logger.warning(f"{mediainfo.title} 无法转换为 TMDB 身份，跳过剧集日期增强")
            return None, None
        episodes = self.tmdb_chain.tmdb_episodes(tmdbid=tmdb_id, season=season)
        if not episodes:
            return None, None
    ...
    def __get_tv_season_air_date(self, mediainfo: MediaInfo, season: int) -> Tuple[Optional[datetime], Optional[str]]:
        ...
        tmdb_id = self.__tmdb_media_id(mediainfo, mtype=MediaType.TV, season=season)
        if not tmdb_id:
            logger.warning(f"{mediainfo.title} 无法转换为 TMDB 身份，跳过季上映日期增强")
            return None, None
        episodes = self.tmdb_chain.tmdb_episodes(tmdbid=tmdb_id, season=season)
        if not episodes:
    ...
    def __get_related_download_histories(self, subscribe_id: Optional[int], subscribe: Subscribe) -> \
        ...
        media_source, media_id = self.__resolve_identity(subscribe)
        if not media_source or not media_id:
            logger.warning(f"订阅 ID {subscribe.id} 身份不完整，跳过下载历史匹配")
            return []
        season = subscribe.season if subscribe_type == MediaType.TV else None
        downloads = self.downloadhistory_oper.get_last_by(
            mtype=subscribe.type,
            season=season,
            media_source=media_source,
            media_id=media_id,
        ...
        for download in downloads:
            ...
            if subscribe_info.get("id") != subscribe_id:
                continue
            info_source, info_media_id = self.__resolve_identity(subscribe_info)
            if info_source != media_source or info_media_id != media_id:
                continue
