...
from app.schemas import Response, ServiceInfo
from app.schemas.types import EventType, MediaType, SystemConfigKey
from app.sdk.config import settings
from app.sdk.events import Event, eventmanager
from app.sdk.logging import logger
from app.sdk.media import Context, MetaInfo
...
class DownloadSiteTagModNew(_PluginBase):
    ...
    def _save_config(self, config_payload: dict) -> Response[dict]:
        ...
        return Response(
            success=True,
            message="配置已成功保存",
            data={"saved_config": self._get_config()}
        )

    def _reset_categories(self) -> Response[dict]:
        """
        重置二级分类
        """
        # 重置为默认分类
            ...
            for torrent in torrents:
                try:
                    ...
                    # 如果站点名称为空, 尝试通过trackers识别
                        ...
                        for tracker in trackers:
                            ...
                            if site_info:
                                ...
                                break
                        # 如果通过tracker还是无法获取站点名称, 且媒体身份、类型、标题都是空的, 那么跳过当前种子
                        if not history.torrent_site and not history.media_id and not history.type and not history.title:
                    ...
                    if service.type == "qbittorrent" and self._enable_category and not torrent_cat and history.type and not self._rename_type:
                        ...
                        if history.media_source and history.media_id and history_type:
                            media_info = self.chain.recognize_media(
                                mtype=history_type,
                                media_source=history.media_source,
                                media_id=history.media_id
                            )
                            cat = media_info.category if media_info else None
    ...
    def download_added(self, event: Event):
        ...
        try:
            ...
            if not services:
                logger.info("触发添加下载事件，但没有可用下载器服务，跳过后续处理")
                return

            service = services.get(downloader)
            if not service:
            ...
            if not context or not context.torrent_info or not context.media_info:
                logger.info("触发添加下载事件，但上下文或媒体信息不完整，跳过后续处理")
                return
            ...
            if (self._enable_category and _media.type and not self._rename_type
                    and _media.media_source and _media.media_id):
                media_info = self.chain.recognize_media(
                    mtype=_media.type,
                    media_source=_media.media_source,
    ...
    def get_cat_rename_by_dict(self, cat:str):
        ...
        _cat = self._cat_rename_dict.get(cat, cat)
