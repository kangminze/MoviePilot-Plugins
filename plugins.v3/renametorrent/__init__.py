    ...
    def torrents_info(self, torrent_hash: Optional[Union[str, list]] = None) -> List[TorrentInfo]:
        ...
        if torrents_info:
            for torrent_info in torrents_info:
                raw_tags = torrent_info.get("tags") or []
                tags = raw_tags if isinstance(raw_tags, list) else [
                    tag.strip() for tag in raw_tags.split(",") if tag.strip()
                ]
                raw_files = getattr(torrent_info, "files", None) or torrent_info.get("files") or []
...
class RenameTorrent(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        # 插件实例会在重载时复用，必须先停止旧任务并清空运行态配置。
        self.stop_service()
        self.downloader = None
        self._cron = ""
        self._cron_enabled = False
        self._event_enabled = False
        self._format_torrent_name = ""
        self._downloader = []
        self._exclude_tags = ""
        self._include_tags = ""
        self._exclude_dirs = ""
        self._hash_white_list = ""
