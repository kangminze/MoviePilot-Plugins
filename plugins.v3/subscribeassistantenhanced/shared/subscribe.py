...
def subscribe_media_identity(subscribe) -> tuple[str, str]:
    """返回订阅的 V3 规范媒体身份；身份不完整时返回空值对。"""
    if not subscribe:
        return "", ""
    media_source = str(subscribe.media_source or "").strip()
    media_id = str(subscribe.media_id or "").strip()
    if not media_source or not media_id:
        return "", ""
    return media_source, media_id


def subscribe_tmdb_id(subscribe) -> Optional[int]:
    """仅把 TMDB 主身份转换为 TMDB API 所需整数 ID，其他来源不跨源猜测。"""
    media_source, media_id = subscribe_media_identity(subscribe)
    if media_source != MediaSource.TMDB.value:
        return None
    try:
...
def match_subscribe(subscribe, task: dict) -> bool:
    ...
    if task.get("name") != subscribe.name:
        return False
    media_source, media_id = subscribe_media_identity(subscribe)
    if task.get("media_source") != media_source or str(task.get("media_id") or "") != media_id:
        return False
