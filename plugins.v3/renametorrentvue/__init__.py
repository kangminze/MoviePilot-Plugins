...
from app.sdk.media import Context, MediaInfo, MetaBase, MetaInfo
...
class RenameTorrentVue(_PluginBase):
        ...
        try:
            if downloader_type == "qbittorrent":
                return {
                    ...
                    "tags": torrent.tags if isinstance(torrent.tags, list) else [
                        tag.strip() for tag in (torrent.tags or "").split(",") if tag.strip()
                    ],
    ...
    def event_process_main(self, event: Event):
        ...
        if self._event_enabled and downloader in (self._downloader if isinstance(self._downloader, list) else [self._downloader]):
            ...
            if not context or not context.meta_info or not context.media_info:
                logger.info("下载事件上下文或媒体信息不完整，跳过重命名")
                return
    ...
    def delete_rename_history(self, request_body: dict):
        ...
        try:
            ...
            if not records_to_delete:
                return Response(success=False, message="未提供要删除的记录")
            ...
            return Response(success=True, message=f"成功删除 {len(hashes_to_delete)} 条记录")

        except Exception as e:
            logger.error(f"删除重命名历史记录失败: {str(e)}")
            return Response(success=False, message=f"删除重命名历史记录失败: {str(e)}")
