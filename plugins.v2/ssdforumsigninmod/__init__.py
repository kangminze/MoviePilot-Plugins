...
class SSDForumSigninMod(_PluginBase):
    # 插件名称
    plugin_name = "SSDForum签到魔改版"
    # 插件描述
    plugin_desc = "(基于imaliang修改)SSDForum自动签到，支持随机延迟。"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/imaliang/MoviePilot-Plugins/main/icons/ssdforum.png"
    # 插件版本
    plugin_version = "1.0"
    # 插件作者
    plugin_author = "imaliang,Seed680"
    # 作者主页
    author_url = "https://github.com/imaliang"
    # 插件配置项ID前缀
    plugin_config_prefix = "ssdforumsigninmod_"
    # 加载顺序
    plugin_order = 3
    # 可使用的用户级别
    auth_level = 1
    ...
    def signin(self, event: Event = None, **kwargs):
        """
        SSDForum签到
        """
        if event:
